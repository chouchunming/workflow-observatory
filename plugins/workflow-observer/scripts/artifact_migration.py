...
def _validate_derived_document(value: object) -> dict[str, object]:
    ...
    source = _exact_mapping(document["source"], _SOURCE_KEYS, "derived source")
...
_Handler = Callable[
    [MarkdownEnvelope, Mapping[str, object] | None], Mapping[str, object]
]
...
def migrate_artifact(
    *,
    ...
    if expected_artifact_type == "learning-snapshot":
        artifact = _declared_json_source(
            source_bytes,
            expected_artifact_type=expected_artifact_type,
            policies=policies,
        )
        migration = _registry_binding(artifact, policies=policies)
        if migration.handler not in _HANDLERS:
            raise ArtifactMigrationError(
                f"unsupported migration handler: {migration.handler}"
            )
        raise ArtifactMigrationError("learning migration dispatch is inconsistent")
    ...
    if value.get("run_id") != source_run_id:
